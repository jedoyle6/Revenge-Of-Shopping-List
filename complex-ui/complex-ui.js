...
function renderShoppingList() {
  ...
  filteredItems = filteredItems.filter(item => item.name.includes(STORE.searchParam));
...
// Places an event listener on the search bar for finding items
// Sets the STORE.searchByString property to the inputted value and re-renders the page
function handleItemSearch() {
  $('#js-list-search-form').submit(function(event) {
    event.preventDefault();
    STORE.searchParam = $('.js-shopping-list-search').val();
    renderShoppingList();
...
//passes ID and new name to editListItem and re-renders the page
function handleItemEdit() {
  $('.js-shopping-list').on('click', '.js-item-edit', event => {
    const newName = window.prompt('New item name:'); //prompts for a new name
    const itemIndex = getItemIdFromElement(event.currentTarget); //gets the id of the targeted element

    editListItem(itemIndex, newName);
    renderShoppingList();

  });
}

function editListItem (itemId, newName) {
